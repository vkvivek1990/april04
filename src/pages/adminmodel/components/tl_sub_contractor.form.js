...
const TLSUBCONTRACTOR = (props) => {
  ...
  // useEffect(() => {
  //   return () => {
  //     dispatchTlSubContractorInitialValues(tl_sub_contractor_init_value);
  //   };
  // }, []);
  ...
  useEffect(() => {
    dispatchCurrentPageType({
      ...
      current_page: "tl_sub_contractor",
    });
    if (!tl_sub_contractor_service_hitted) {
      const onLoadService = async () => {
        await getFormFields("tl_sub_contractor");
  ...
  const onSubmit = () => {
    ...
    if (error) {
      ...
      submitRegistration("tl_sub_contractor")
        .then((res) => {
          dispatchenableServiceMenu(true);
          if (arr_comple_Forms.indexOf("tl_sub_contractor") < 0) {
            arr_comple_Forms.push("tl_sub_contractor");
          }
          dispatchCurrentPageType({
            completed_forms: arr_comple_Forms,
          });
          let assignId = { ...tl_sub_contractor, id: res.payLoad.id };
          dispatchFormInitialValues({ tl_sub_contractor: assignId });
...
export default TLSUBCONTRACTOR;
